...
scriptVersion="1.2.0"
# Icon
icon="https://is1-ssl.mzstatic.com/image/thumb/Purple113/v4/df/9b/84/df9b84e6-e209-23d1-4423-1102bfc5473c/AppIcon-0-0-1x_U007emarketing-0-0-0-7-0-0-sRGB-0-0-0-GLES2_U002c0-512MB-85-220-0-0.png/460x0w.webp"
...
overlayicon="${7:-"/System/Library/CoreServices/CoreTypes.bundle/Contents/Resources/FileVaultIcon.icns"}"
supportInformation="${8:-"https://saks.atlassian.net/servicedesk/customer/portal/6"}"
# Banner image for message
overlayiconsuccess="${9:-"/Users/Shared/checkmark.png"}"
banner="${10:-"https://live.staticflickr.com/65535/52843232549_f2caea6cbf_h.jpg"}"
## SwiftDialog
...
FAIL_MESSAGE="## Please check your password and try again.\n\nIf issue persists, please contact support at: $supportInformation"
SUCCESS_MESSAGE="## FileVault Recovery Key was successfully reissued and stored."
...
--ontop \
--moveable \
--textfield \"Enter Mac Login Password\",secure,required"
...
--bannerimage \"$banner\" \
--icon \"$icon\" \
--overlayicon \"$overlayiconsuccess\" \
...
userPass=$(eval "$dialogCMD" | grep "Enter Mac Login Password" | awk -F " : " '{print $NF}')
...
until /usr/bin/dscl /Search -authonly "$userName" "${userPass}" &>/dev/null; do
	...
	userPass=$(eval "$dialogForgotCMD" | grep "Enter Mac Login Password" | awk -F " : " '{print $NF}')
	if (( TRY >= 5 )); then
