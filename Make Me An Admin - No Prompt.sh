...
#osascript -e 'display dialog "You now have administrative rights for 30 minutes.

#Please only conduct the tasks for which you have been approved to have this access.

#Actions made as admin will be logged. If any unapproved tasks or actions are found to have been taken, future admin access may no longer be granted.." buttons {"I understand -  Make me an admin"} default button 1'
...
#Set the run inverval to run after 90 minutes
sudo defaults write /Library/LaunchDaemons/removeAdmin.plist StartInterval -integer 5400
...
if [[ -f /private/var/userToRemove/user ]]; then
	...
	log collect --last 90m --output /private/var/userToRemove/$userToRemove.logarchive
fi
EOF
