...
if [ -f /Library/LaunchDaemons/saks.postrestart.reconAtReboot.plist ];then
    echo "LaunchDaemon that runs recon on startup exists will exit out"
    exit 0
else
    echo "LaunchDaemon that runs recon on startup DOES NOT exists will proceed with the script"
fi
...
scriptVersion="1.0.1"
plistDomain="saks.postrestart"       # Hard-coded domain name
